...
{
    add_motor_num(AP_MOTORS_MOT_1);       // front right motor
    add_motor_num(AP_MOTORS_MOT_2);       // front left motor
    add_motor_num(AP_MOTORS_MOT_3);       // front motor
    add_motor_num(AP_MOTORS_MOT_4);       // tail motor
...
//#endif
    ...
    set_initialised_ok(frame_class == MOTOR_FRAME_F35B);     // was: MOTOR_FRAME_TRI
    //_flags.initialised_ok = (frame_class == MOTOR_FRAME_F35B);
}
...
void AP_MotorsF35B::output_armed_stabilizing()
{
    ...
    _thrust_right = roll_thrust * -1.0f;  //_thrust_right = roll_thrust * -0.5f + pitch_thrust * 0.5f;
    _thrust_left = roll_thrust * 1.0f;    //_thrust_left = roll_thrust * 0.5f + pitch_thrust * 0.5f;
    ...
    rpy_low = MIN(MIN(_thrust_right, _thrust_left), _thrust_front);   //rpy_low = MIN(_thrust_right, _thrust_left);
    rpy_high = MAX(MAX(_thrust_right, _thrust_left), _thrust_front);  //rpy_high = MAX(_thrust_right, _thrust_left);
    ...
    _thrust_right = (_throttle_thrust_max + _thrust_right) * 0.5f;   //_thrust_right = throttle_thrust_best_plus_adj + rpy_scale * _thrust_right;
    _thrust_left = (_throttle_thrust_max + _thrust_left) * 0.5f;     //_thrust_left = throttle_thrust_best_plus_adj + rpy_scale * _thrust_left;
...
void AP_MotorsF25B::output_test_seq(uint8_t motor_seq, int16_t pwm)
{
